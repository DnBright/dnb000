...
<div class="reveal active space-y-10">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
            <h2 class="text-3xl font-black text-white tracking-tight uppercase leading-none">Dashboard <span class="animate-text-shimmer">Services</span></h2>
            <div class="flex items-center gap-3 mt-4">
                <span class="text-[10px] text-slate-500 font-bold uppercase tracking-[0.3em]">Protocol: Configuration Module 05 (Home Services)</span>
    ...
    @if(session('success'))
        <div class="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-[10px] font-black text-emerald-400 uppercase tracking-widest animate-fade-in-up">
            Transaction Complete: Popular Services successfully synchronized.
        </div>
    @endif
    ...
    <form method="POST" action="{{ route('admin.pages.services.update') }}" enctype="multipart/form-data" class="space-y-10">
        @csrf
        @method('PUT')

        <div class="flex items-center justify-between border-b border-white/5 pb-6">
            <p class="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em]">Deployment: 06 Strategic Service Nodes</p>
        </div>
                ...
                <div class="glass-card p-8 space-y-6 relative group overflow-hidden">
                    ...
                    <div class="space-y-4">
                        <div class="group">
                            ...
                            <input name="service_titles[]" value="{{ old('service_titles.'.$i, $svc['title'] ?? $defaults[$i]) }}" class="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-5 text-xs text-white focus:outline-none focus:border-brand-cyan/50 transition-all font-bold">
                        </div>

                        <div class="group">
                            <label class="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-3 block ml-1">Sub-Directive summary</label>
                            <input name="service_subtitles[]" value="{{ old('service_subtitles.'.$i, $svc['subtitle'] ?? '') }}" class="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-5 text-xs text-slate-400 focus:text-white focus:outline-none focus:border-brand-cyan/50 transition-all font-medium">
                        </div>

                        <div class="group">
                            <label class="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-3 block ml-1">Modal Detail Protocol (Expanded)</label>
                            <textarea name="service_descriptions[]" rows="3" class="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-6 text-xs text-slate-400 focus:text-white focus:outline-none focus:border-brand-cyan/50 transition-all font-medium h-24 resize-none">{{ old('service_descriptions.'.$i, $svc['description'] ?? '') }}</textarea>
                        </div>
                    </div>

                    <div class="space-y-4 pt-2">
                        <label class="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] block ml-1">Node Asset (Visual)</label>
                        ...
                        @if(!empty($svc['image']))
                            <div class="relative w-full h-32 rounded-xl overflow-hidden border border-white/10 group-hover:border-brand-cyan/30 transition-all">
                                <img src="{{ $svc['image'] }}" class="w-full h-full object-cover">
                                <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                                <div class="absolute bottom-2 left-3 text-[8px] text-white/60 font-mono truncate max-w-[90%]">{{ basename($svc['image']) }}</div>
                            </div>
                        @else
                            <div class="w-full h-20 rounded-xl bg-white/5 border border-dashed border-white/10 flex items-center justify-center">
                                <span class="text-[8px] text-slate-600 font-black uppercase tracking-widest">No Asset Injected</span>
                            </div>
                        @endif

                        <div class="flex items-center gap-4">
                            <div class="relative flex-1">
                                <input type="file" name="service_images[]" accept="image/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" data-index="{{ $i }}">
                            ...
                            <label class="flex items-center gap-3 cursor-pointer group px-4 py-2 bg-white/5 rounded-xl border border-white/5 hover:border-red-500/30 transition-all">
                                <input type="checkbox" name="remove_service_{{ $i }}" value="1" class="hidden peer">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-end gap-4 pt-8 border-t border-white/5">
            ...
            <button type="submit" class="btn-primary py-4 px-10 text-[10px] uppercase tracking-[0.2em] font-black w-full sm:w-auto shadow-lg shadow-brand-cyan/20">Commit Services Roster</button>
        </div>
    </form>
</div>
...
<script>
    (function(){
        document.querySelectorAll('input[name="service_images[]"]').forEach(function(inp){
            inp.addEventListener('change', function(e){
                ...
                const removeCb = document.querySelector(`input[name="remove_service_${idx}"]`);
