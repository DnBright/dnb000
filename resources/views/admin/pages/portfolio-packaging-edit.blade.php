...
<div class="reveal active space-y-10">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
            <h2 class="text-3xl font-black text-white tracking-tight uppercase leading-none">Packaging <span class="animate-text-shimmer">Matrix</span></h2>
            <div class="flex items-center gap-3 mt-4">
                <span class="text-[10px] text-slate-500 font-bold uppercase tracking-[0.3em]">Protocol: Configuration Module 07-P</span>
                <span class="w-1 h-1 bg-slate-700 rounded-full"></span>
                <span class="text-[10px] text-brand-cyan font-bold uppercase tracking-[0.3em]">Physical Asset Registry</span>
            </div>
        </div>
    ...
    @if(session('success'))
        <div class="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-[10px] font-black text-emerald-400 uppercase tracking-widest animate-fade-in-up">
            Transaction Complete: Packaging Matrix successfully synchronized.
        </div>
    @endif

    <form method="POST" action="{{ route('admin.portfolio.packaging.update') }}" enctype="multipart/form-data" class="space-y-10">
        @csrf
        @method('PUT')

        <div class="flex items-center justify-between border-b border-white/5 pb-6">
            <p class="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em]">Deployment: 03 Strategic Packaging Nodes</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            @for($i=0;$i<3;$i++)
                @php $it = $content['packaging'][$i] ?? []; @endphp
                <div class="glass-card p-6 space-y-6 relative group overflow-hidden">
                    ...
                    <div class="space-y-4">
                        <div class="group">
                            ...
                            <input name="packaging_titles[]" value="{{ old('packaging_titles.'.$i, $it['title'] ?? '') }}" class="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-5 text-xs text-white focus:outline-none focus:border-brand-cyan/50 transition-all font-bold">
                        </div>

                        <div class="group">
                            <label class="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-3 block ml-1">Sub-Directive summary</label>
                            <input name="packaging_captions[]" value="{{ old('packaging_captions.'.$i, $it['caption'] ?? '') }}" class="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-5 text-xs text-slate-400 focus:text-white focus:outline-none focus:border-brand-cyan/50 transition-all font-medium">
                        </div>

                        <div class="group">
                            <label class="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-3 block ml-1">Operational Dossier</label>
                            <textarea name="packaging_descriptions[]" rows="3" class="w-full bg-white/5 border border-white/10 rounded-xl py-4 px-6 text-xs text-slate-400 focus:text-white focus:outline-none focus:border-brand-cyan/50 transition-all font-medium h-24 resize-none">{{ old('packaging_descriptions.'.$i, $it['description'] ?? '') }}</textarea>
                        </div>

                        <div class="group">
                            <label class="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-3 block ml-1">Access Protocol (URL)</label>
                            <input name="packaging_links[]" value="{{ old('packaging_links.'.$i, $it['link'] ?? '') }}" class="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-5 text-[10px] text-slate-500 focus:text-white focus:outline-none transition-all font-mono">
                        </div>
                    </div>
                    ...
                    <div class="space-y-4 pt-2">
                        ...
                        <div class="flex items-center gap-4">
                            <div class="relative flex-1">
                                <input type="file" name="packaging_images[]" accept="image/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" data-index="{{ $i }}">
                            ...
                            <label class="flex items-center gap-3 cursor-pointer group px-4 py-2 bg-white/5 rounded-xl border border-white/5 hover:border-red-500/30 transition-all">
                                <input type="checkbox" name="remove_packaging_{{ $i }}" value="1" class="hidden peer">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-end gap-4 pt-8 border-t border-white/5">
            ...
            <button type="submit" class="btn-primary py-4 px-10 text-[10px] uppercase tracking-[0.2em] font-black w-full sm:w-auto shadow-lg shadow-brand-cyan/20">Commit Packaging Matrix</button>
        </div>
    </form>
</div>
...
<script>
    (function(){
        document.querySelectorAll('input[name="packaging_images[]"]').forEach(function(inp){
            inp.addEventListener('change', function(e){
                ...
                const removeCb = document.querySelector(`input[name="remove_packaging_${idx}"]`);
